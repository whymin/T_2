...
def error(formatted_string):
    print
    'ERROR - %s' % formatted_string


def debug(formatted_string):
    if DEBUG:
        print
        'DEBUG - %s' % formatted_string
...
def is_string_load(addr):
    ...
    if (GetMnem(addr) != 'mov' and GetMnem(addr) != 'lea') and (
            GetOpType(addr, 1) != 2 or GetOpType(addr, 1) != 5) or GetOpnd(addr, 1)[-4:] == '_ptr':
        return False
    ...
    if GetOpnd(addr, 0) in VALID_REGS and not ('[' in GetOpnd(addr, 1) or 'loc_' in GetOpnd(addr, 1)) and (
            ('offset ' in GetOpnd(addr, 1) or 'h' in GetOpnd(addr, 1)) or ('unk' == GetOpnd(addr, 1)[:3])):
        ...
        try:
            ...
            return False
        if GetMnem(addr_2) == 'mov' and dest_reg in VALID_DEST and ('[%s' % dest_reg) in GetOpnd(addr_2, 0) and GetOpnd(
                addr_2, 1) == from_reg:
            ...
            if GetMnem(addr_3) == 'mov' and (('[%s+' % dest_reg) in GetOpnd(addr_3, 0) or GetOpnd(addr_3,
                                                                                                  0) in VALID_DEST) and 'offset ' not in GetOpnd(
                addr_3, 1) and 'dword ptr ds' not in GetOpnd(addr_3, 1) and GetOpType(addr_3, 1) != 1 and GetOpType(
...
def traverse_xrefs(func):
    ...
    while func_xref != BADADDR:
        ...
        if idaapi.get_func(func_xref) is None:
            ...
            if idaapi.get_func_name(xref_func.startEA) is not None and 'sub_' not in idaapi.get_func_name(
                    xref_func.startEA):
